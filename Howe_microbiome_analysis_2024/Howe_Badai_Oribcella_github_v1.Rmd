...
author: "Colin Howe, Sofia Roitman, & Amanda Badai"
date: "`r Sys.Date()`"
...
setwd("C:/Users/Colin Howe/Desktop/PSU Doctoral Program/GCMP_Working/Robustness-trade-offs")
...
##Rarefication this will be used for statistical comparative analysis at 5000 reads depth we retained 24.5% of the reads and 89% of the sample data
...
pcoa1<-0
pcoa1 <- plot_ordination(ofra, rt.pcoa1, color="depth", title =expression(paste(italic(" Orbicella franski")," Weighted Unifrac Distance PCoA across Depth"))) + theme_bw() + stat_ellipse(level = 0.95,type = "norm",aes(group=depth)) + theme(legend.text=element_text(size=12)) + scale_color_manual(values=c("blue1","#1F78B4"))+ geom_point(size=3) +  theme(plot.title = element_text(hjust = 0.5))  + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold")))


## All Ofav
rt.pcoa2 = ordinate(ofav, method="PCoA", distance=wuni_ofav)
pcoa2<-0
pcoa2 <- plot_ordination(ofav, rt.pcoa2, color="depth", title =expression(paste(italic(" Orbicella faveolata")," Weighted Unifrac Distance PCoA across Depth"))) + theme_bw() + stat_ellipse(level = 0.95,type = "norm",aes(group=depth)) + scale_color_manual(values=c("maroon","red1"))+ geom_point(size=3) +  theme(plot.title = element_text(hjust = 0.5))  + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold")))# 
